...
export function ClienteForm({ cliente, onClose }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => clientesService.update(cliente.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['clientes']);
      onClose();
    }
                      ...
                      {error && (
                        <div className="rounded-md bg-red-900 bg-opacity-50 p-4">
                          <p className="text-sm text-red-400">
                            Error al {cliente ? 'actualizar' : 'crear'} el cliente. Por favor, intente nuevamente.
                          </p>
                        </div>
                      )}
